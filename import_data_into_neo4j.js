...
for(let link of links){
    resultPromise = session.run(
        'MATCH (j:Intervention {id: $node1}) MATCH (m:Outcome {id: $node2}) MERGE (j)-[r:Intervent {field: $field}]->(m) RETURN j, r, m',
...
// 批量添加关系
// resultPromise = session.run(
//     `UNWIND $links AS link MATCH (a {id:link.source}) MATCH (b{id:link.target }) MERGE (a)-[:${label} {id: link.id}]->(b) ` ,
